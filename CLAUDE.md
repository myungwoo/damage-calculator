...
- **총 틱 수**: 만료시각에 `tDelay`가 더해지는지 확정할 수 없어 지속시간과 같은 수
  (만렙 4틱)로 뒀다. `durationSeconds` 사용부만 고치면 바꿀 수 있다.
  한 번 때리고 지켜본 실측에서 틱이 3~6회(보통 4~5회)로 흔들렸는데,
  서버 로직만으로는 이 편차가 설명되지 않는다. 화면에 뜨는 숫자는 클라이언트가
  그리는 것이라 서버의 HP 차감과 개수가 어긋날 수 있고, HP가 1에서 멈춘 뒤의 틱은
  깎을 HP가 없어 보이지 않을 수 있다. 이쪽이 원인일 가능성이 크다.
